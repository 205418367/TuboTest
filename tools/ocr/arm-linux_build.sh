...
ARM_ABI="LINARO" # AARCH64 YINGLAIARM YINGLAINPU LINARONPU LINAROCPU OPENWRT JBOLONG
PROJECT_PATH=`pwd`
...
if [ "$ARM_ABI" = "AARCH64" ];then
    ...
    export TOOLCHAIN=/media/lichen/4T-data/workspace/3third/uodizh
fi
...
if [ "$ARM_ABI" = "OPENWRT" ] || [ "$ARM_ABI" = "OPENWRTNPU" ];then
    ...
    export LDFLAGS="-L${TOOLCHAIN}/lib -L${TOOLCHAIN2}/usr/lib -static-libstdc++ -Wl,-rpath=${TOOLCHAIN}/lib"
else
    export CXXFLAGS="$CXXFLAGS -Os -fPIC -D_GLIBCXX_USE_CXX14_ABI=1 -I${TOOLCHAIN}/include"
    export LDFLAGS="-L${TOOLCHAIN}/lib -static-libgcc -static-libstdc++ -Wl,-rpath=${TOOLCHAIN}/lib"
fi
